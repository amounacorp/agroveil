...
function exportCSV(farmers: Farmer[]) {
  ...
  a.href = url;
  a.download = `Aviora-eleveurs-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[#1A1A1A]">{t.farmers.title}</h2>
          <p className="text-sm text-[#888888] mt-0.5">
            {data?.total ?? '—'} · {t.farmers.subtitle}
          </p>
        </div>
        <button
        ...
        >
          <Plus size={16} /> {t.farmers.addFarmer}
        </button>
      </div>
      ...
      <div className="bg-white rounded-card border border-[#E8E8E8] shadow-card px-5 py-4 mb-4 flex flex-wrap gap-3 items-center">
        ...
        >
          <Download size={15} /> {t.common.export} CSV
        </button>
      </div>
