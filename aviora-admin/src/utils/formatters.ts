...
import { CURRENCY_RATES, type Currency } from '../i18n/translations';

export function formatFCFA(amount: number): string {
  return amount.toLocaleString('fr-FR').replace(/\s/g, ' ') + ' FCFA';
}

export function formatCurrency(amountXAF: number, currency: Currency = 'XAF'): string {
  const value = Math.round(amountXAF * CURRENCY_RATES[currency]);
  if (currency === 'USD') return '$' + value.toLocaleString('en-US');
  if (currency === 'EUR') return value.toLocaleString('fr-FR').replace(/\s/g, ' ') + ' €';
  return amountXAF.toLocaleString('fr-FR').replace(/\s/g, ' ') + ' FCFA';
}
