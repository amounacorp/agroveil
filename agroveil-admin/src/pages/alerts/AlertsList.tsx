...
export default function AlertsList() {
  ...
  const resolveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast('Alerte résolue avec succès', 'success');
    },
  ...
  const whatsappMutation = useMutation({
    ...
    onSuccess: () => addToast('Message WhatsApp envoyé', 'success'),
  ...
  return (
    <AdminLayout>
      <div className="flex items-end justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[#1A1A1A]">Alertes</h2>
          <p className="text-sm text-[#888888] mt-0.5">
            {data?.total ?? '—'} alertes · Surveiller et résoudre
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-[#F8FAF8] border-b border-[#E8E8E8]">
                <tr className="text-[10px] text-[#888888] uppercase font-bold tracking-wider">
                  ...
                  <th className="px-5 py-3">Ferme</th>
