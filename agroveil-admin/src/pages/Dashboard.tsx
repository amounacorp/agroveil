...
import { formatRelativeDate, formatAlertType, formatFCFA } from '../utils/formatters';
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[#1A1A1A]">Vue d'ensemble</h2>
          <p className="text-sm text-[#888888] mt-0.5">
            Surveillance en temps réel du parc avicole AgroVeil.
          </p>
        </div>
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <KPICard
            ...
            sub={`${stats?.critical_alerts ?? 0} critiques`}
          ...
          <KPICard
            label="MRR"
            value={stats ? formatFCFA(stats.mrr_fcfa) : '—'}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="lg:col-span-2 bg-white rounded-card border border-[#E8E8E8] shadow-card p-5">
          <div className="flex items-center justify-between mb-5">
            <h4 className="font-bold text-[#1A1A1A]">Croissance Éleveurs</h4>
        ...
        <div className="bg-white rounded-card border border-[#E8E8E8] shadow-card p-5">
          <h4 className="font-bold text-[#1A1A1A] mb-5">Répartition Plans</h4>
      ...
      <div className="bg-white rounded-card border border-[#E8E8E8] shadow-card overflow-hidden mb-6">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E8E8E8]">
          <h4 className="font-bold text-[#1A1A1A]">Alertes Récentes</h4>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-[#F2F2F2]">
              {alerts.map((alert) => (
                ...
                >
                  <td className="px-5 py-3.5 font-semibold text-sm text-[#1A1A1A]">
                    {alert.farmer_name}
                  </td>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-xs font-semibold text-[#555555]">Statut Système :</span>
        <span className="flex items-center gap-1.5 text-xs font-medium text-[#27500A] bg-[#EAF3DE] px-3 py-1 rounded-pill">
          <span className="w-1.5 h-1.5 bg-[#1E6B2E] rounded-full" /> API en ligne
        </span>
        <span className="flex items-center gap-1.5 text-xs font-medium text-[#27500A] bg-[#EAF3DE] px-3 py-1 rounded-pill">
          <span className="w-1.5 h-1.5 bg-[#1E6B2E] rounded-full" /> Base de données
        </span>
