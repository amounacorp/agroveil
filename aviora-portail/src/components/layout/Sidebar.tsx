...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { to: '/alertes',     icon: Bell,            label: t.nav.alerts,       exact: false },
    { to: '/rapports',    icon: BarChart2,       label: t.nav.reports,      exact: false },
    { to: '/abonnement',  icon: CreditCard,      label: t.nav.subscription, exact: false },
    { to: '/parametres',  icon: Settings,        label: t.nav.settings,     exact: false },
  ];
  ...
  return (
    <nav className="hidden md:flex flex-col h-screen fixed left-0 top-0 w-60 bg-[#0F3D1A] shadow-lg z-50">
      ...
      <div className="px-6 py-5 border-b border-white/10">
        ...
        <p className="text-xs text-white/50 uppercase tracking-widest">{t.nav.farmerPortal}</p>
      </div>
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-[10px] text-white/50 uppercase">{t.nav.owner}</p>
          </div>
        </div>
        <button
