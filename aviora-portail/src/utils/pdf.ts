...
export async function generateReportPDF(
  ...
  doc.setFontSize(8);
  doc.setTextColor(136, 136, 136);
  doc.text('Aviora — Portail Fermier Intelligent', 14, pageH - 5);
  doc.text(`${farm.name} · ${report.month_label}`, pageW - 14, pageH - 5, { align: 'right' });

  const filename = `Aviora_Rapport_${report.month_label.replace(' ', '_')}_${farm.name.replace(/\s/g, '_')}.pdf`;
  doc.save(filename);
}

export function shareOnWhatsApp(report: MonthlyReport, farm: Farm): void {
  const msg = encodeURIComponent(
    `📊 *Rapport Aviora — ${report.month_label}*\n` +
    `🏠 ${farm.name}\n\n` +
