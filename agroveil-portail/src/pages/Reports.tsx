...
export function Reports() {
  ...
  const { data: reports, isLoading } = useReports(mockFarm.id);
  const { data: alerts } = useAlerts(mockFarm.id);
  const generateReport = useGenerateReport(mockFarm.id);

  const handleDownload = async (report: typeof reports extends (infer R)[] | undefined ? R : never) => {
    try {
      await generateReportPDF(report as Parameters<typeof generateReportPDF>[0], mockFarm, alerts ?? []);
